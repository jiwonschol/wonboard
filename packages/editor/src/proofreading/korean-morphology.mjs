...
export function createMorphology(sets) {
  ...
  function predicate(s) {
    ...
    for(let i=1;i<s.length;i++)if(prefixes.has(s.slice(0,i))&&sets.ending.has(s.slice(i)))return {root:prefixes.get(s.slice(0,i)),adnominal:/^(는|은|던|을)$/.test(s.slice(i))};
    for(let i=1;i<s.length;i++) {
      if(!sets.noun.has(s.slice(0,i)))continue;
      const suffix=s.slice(i);
      let tail=forms.get(suffix);
      if(!tail)for(let j=1;j<suffix.length;j++)if(prefixes.get(suffix.slice(0,j))==='하'&&sets.ending.has(suffix.slice(j)))tail={root:'하',adnominal:/^(는|은|던|을)$/.test(suffix.slice(j))};
  ...
  function analyze(s,personal,unknown=false) {
    ...
    if(sets.adverb.has(s))return {kind:'adverb',cost:1};
  ...
  function spacing(word,personal) {
    ...
    if(word.length<=6&&unknownNoun(word))return null;
