...
동준의 실행 환경(Linux VPS)과 다르다. 아래 6장의 격리 결함은 이 차이에서만 드러난다.

## 2. 코드 변경 범위 — 재점검 표는 현재 main에서도 그대로 참이다

재점검의 base는 `aa85253`이었다. 그 사이 main에 PR #14(글 휴지통)와 PR #15(Sites 안내 문서)가 병합됐다. 검사 코드 변경 여부를 먼저 쟀다.
...
`icons.tsx`의 차이는 trash 아이콘 한 줄이다(`+  trash: <path d="M4 6h16M9 6V3h6v3M6 6l1 15h10l1-15M10 10v7M14 10v7" />`). `packages/editor/src/proofreading/*`, `scripts/eval-spelling.mjs`, `scripts/spelling-prototype.mjs`, `tests/fixtures/spelling/*`, `third_party/spelling/generated/*` 는 **변경이 없다.**
...
영어 게이트는 이미 통과(28/30 = 93.3%, 분리 9/10 = 90.0%)라서 이 두 건은 완료를 막지 않는다. **미달로 기록만 되고 게이트를 막지 않는 항목**이다. 재점검 코멘트가 "정상 단어의 문맥 중의성이 있는 항목을 검토 없이 강제 교정 규칙으로 만들지 않는다"고 적은 것과 같은 판단이다. 사전 수록 사실과 이 문맥에서의 정답은 구분해서 기록하고, 답안은 건드리지 않는다.
...
**그러므로 해시를 바꿔 통과시키는 것은 해법이 아니다.** 상수를 새 값으로 갈아끼우면 다음 데이터 갱신에서 같은 일이 다시 생긴다. 맞는 direction은 staging 쪽이 policy 검사와 같은 단일 출처를 읽게 하거나 그 검사를 호출하게 하는 것이다. `scripts/stage-proofreading-bundle.mjs`는 제 소유 범위(`packages/editor/src/proofreading`, 맞춤법 시험·평가 기록, `scripts/proofreading-lab`) 밖이고 라이선스 게이트를 건드리므로 **여기서는 판단만 기록하고 수정하지 않는다.**
...
재현 해시(원본 CSV에서 `morphology.json`을 다시 만들어 38a8386e…를 얻는 것)는 **실행하지 않았다.** `build-korean-morphology-data.mjs`가 pinned revision의 CSV를 네트워크로 받기 때문이다. 위 출처 판단은 추적된 README·COPYING·정책 게이트 해시 대조로 충분하다고 봤고, 재현이 필요하면 별도 승인으로 진행한다.
...
수정과 검증은 [PR #19](https://github.com/jiwonschol/wonboard/pull/19)(head `08e7424c60be346dd8191841fb765b2b6f866e3b`)에 있다. 요지만:
...
양성 증명               응답 usage.input_tokens 가 픽스처값 100이고 thread.started 없음

가짜 실행기 결과 (구독 0회)   --test 9/9 2.4초 · pnpm test 346/346 종료 0 · typecheck 종료 0
                            실행 후 새 임시 디렉터리 6개에 실제 CLI 응답 0건,
                            PATH 뒤쪽 감시 실행기는 기록 파일 자체가 미생성(역참조 0건)
...
## 10. 남은 구현 범위와 결정이 필요한 항목

**측정 없이 착수하면 안 되는 것 (순서대로)**

1. 부사+명사 경계 규칙의 폭발 반경 측정 — 기존 200문장·노출 개발 자료 2,122 고유행에서 새 분할 건수와 정상 오제안 19/1028의 변화. 데이터 증가 0바이트지만 규칙 범위는 미확정(5-2)
2. ㅐ/ㅔ 혼동 후보 경로의 근거와 반례 — `연애인` 한 문장 치환 금지(5-1)
3. 독립 평가의 판정 계획 — 기존 자료를 노출시키지 않는 방식. 보류 4,206행 중 미판정 641행(6장)

**지원의 판단이 필요한 것**

- 영어 `accidently`·`calender`: 사전 수록 사실과 이 문맥의 정답을 갈라야 한다. 목록 제거 / 문맥 계층 / 답안 수정 중 어느 것도 제가 정하지 않는다(5-3). 게이트는 막지 않는다
- `stage-proofreading-bundle.mjs`의 해시 상수 drift: 상수 교체가 아니라 단일 출처화로 고칠지, 그리고 그 파일을 누구 소유로 할지(8장)
...
- morphology 재현 해시(원본 CSV → 38a8386e…)를 만들지 않았다
