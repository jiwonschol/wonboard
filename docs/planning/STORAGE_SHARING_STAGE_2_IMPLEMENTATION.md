...
## 최신 리뷰 세 항목 보완 — 2026-09-18

4042254436: 파일 선택 패널의 메타데이터와 현재 글의 파일/사진을 합쳐 개수·220MiB 총 첨부 용량을 먼저 검사한다. 이미 삽입된 같은 ID는 중복 계산하지 않고, 내려받은 메타데이터로도 매번 재검사해 한도를 넘으면 추가 로딩을 멈춘다. 한도 초과 선택은 원본 요청 없이 거절하며 한·영으로 실제 첨부 한도를 안내한다. 오류 안내는 자동으로 보이는 위치에 이동한다.
4042254443: 전송 결과를 알 수 없는 응답 유실은 준비한 업로드 ID를 보존한다. `StorageConflict`로 확정 거절되면 준비값을 해제해 다음 실패 업로드 재시도는 새 ID를 쓴다. 응답 유실 후 정상 재시도에서 같은 ID 유지, 응답 유실 뒤 보관함 파일이 삭제돼 이전 ID가 영구 거절된 경우의 새 ID 성공을 각각 검증했다.
4042254447: 기존 데이터 참조 등록은 파일마다 SQL을 쌓지 않는다. JSON 집합 조회로 파일 hash/size 일치를 확인하고 한 SQL로 그 글의 참조를 삽입한다. 진행 체크포인트를 포함한 페이지의 SQL은 최대 13개이며, 현재 글 revision·진행 revision 조건과 트랜잭션 복귀를 유지한다. 파일 100개씩 가진 글 25개의 참조 2,500개와 기존 공개 객체 26개를 여러 페이지로 빠짐없이 등록하는 반례를 검증했다. [D1 공식 제약](https://developers.cloudflare.com/d1/platform/limits/)을 참고했으며 플랫폼 전체 호출 계약·실제 Site 검증을 대신 주장하지 않는다.

최종 타입 검사, 전체 단위 Vitest 226 + Node 363 = 589개, Sites Chromium/Firefox/WebKit 각 31개 = 93/93, Sites 빌드가 통과했다. 일반 작성기 전체 Chromium 173/173도 통과했다. 이 실행 중 마지막 변경은 오류 안내 스크롤의 nearest → center 한 줄이며 변경 뒤 일반 파일 선택/삽입/취소 3/3과 전체 Sites 93/93을 재검증했다. 한·영×390/1440px 한도 안내 bbox/전체 세로 가시성, 원본 요청 0회와 모바일 화면 직접 확인을 포함한다. 첫 안내 위치 검사는 영어 데스크톱 두 케이스에서 91/93으로 실패했고, center 수정 뒤 해당 12/12를 먼저 통과한 후 전체 93/93을 통과했다. 로그는 `/private/tmp/wonboard-pr23-followup-{types-final,units-final,sites-93,writer-final,file-library-3,build-final,limit-12}.log`다. 이번 검증은 로컬 SQLite/객체 저장소/인증 대역과 macOS 브라우저이며 운영·설치 패키지·Windows 게이트는 남아 있다.

## 재시도 진행과 Undo 바이트 보존 — 2026-09-18

4042365680: 클라이언트 참조 등록은 무조건 초기화하지 않는다. 서버도 진행 중인 작업은 체크포인트부터 이어가며, 완료됐지만 실패 기록이 남은 작업만 다시 등록한다. 초기화와 실패 삭제에는 진행 버전 조건을 둔다. 실제 클라이언트 어댑터를 통해 25개 기존 글과 배포 참조의 반복 재시도 완료, 실패 글 복구 후 재등록, 완료된 정상 작업의 반복 호출을 검증했다.

4042365685: Sites 저장은 열린 편집 세션의 Undo에 필요한 파일 메타데이터/바이트 참조를 함께 보존한다. 보관함에서 영구 삭제하고 링크를 지운 뒤 정리가 실행돼도 Undo로 복원한 글을 다시 저장할 수 있다. 다음 편집 세션은 내용에서 사용하지 않는 파일을 로딩 전에 제거하고, 그 세션의 다음 저장에서 서버 참조도 해제한다. 따라서 사용하지 않는 바이트는 다음 세션 저장까지 보수적으로 보존될 수 있다. 원래 파일 ID와 공유 철회 상태는 바꾸지 않는다.

전체 단위 Vitest 228 + Node 363 = 591개, Sites Chromium/Firefox/WebKit 각 32개 = 96/96, 타입 검사를 포함한 Sites 빌드가 통과했다. 세 브라우저에서 보관함 영구 삭제 → 글 링크 삭제/저장 → 정리 → 실제 Undo → 저장 → 새로고침과 파일 바이트 유지까지 확인했다. 단위 검증은 새 세션 저장 뒤 바이트 정리도 포함한다. 최초 타입 검사는 테스트의 선택적 커서 타입 때문에 실패했고 non-null 표시 후 빌드의 타입 검사까지 통과했다. 로그: `/private/tmp/wonboard-pr23-undo-{target,units,sites,build}.log`. 운영·설치 패키지·Windows 검증 범위는 앞 절과 같다.
