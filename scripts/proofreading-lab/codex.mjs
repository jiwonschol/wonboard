...
export async function createCodex({env=process.env,run=execute}={}){
  ...
  const status=await run('codex',['login','status'],{env:cleanEnv,timeout:10000,maxBuffer:64000});
  ...
  return async(model,body)=>{
    ...
    try{
      ...
      const result=await new Promise((resolve,reject)=>{
        const child=execFile('codex',args,{cwd:dir,env:cleanEnv,timeout:180000,killSignal:'SIGKILL',maxBuffer:2*1024*1024},(error,stdout,stderr)=>{if(error){const failure=Error('Codex invocation failed; no automatic retry');failure.diagnostic={exitCode:error.code,signal:error.signal,stdout,stderr};reject(failure);}else resolve(stdout);});
