...
const revision='12439fb32808b9244ded56d7dfed96e4b8d76869',sources=[],forms=new Map(),adverbs=new Set();
for(const file of ['Inflect.csv','MAG.csv']){
...
const payload=JSON.stringify({notice:'Modified by Wonboard: selected Hangul verb inflections and adverbs; stripped engine costs/IDs. Original Apache-2.0 terms: ../mecab-ko-dic/COPYING.',forms:[...forms.values()],adverbs:[...adverbs].sort()})+'\n';
...
const manifest={revision,sources,acquired:new Date().toISOString(),bytes:Buffer.byteLength(payload),gzipBytes,combinedGzipBytes:gzipBytes+existing,forms:forms.size,adverbs:adverbs.size};
