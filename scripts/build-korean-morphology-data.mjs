...
for(const file of ['Inflect.csv','MAG.csv','NNG.csv','Foreign.csv']){
  ...
  for(const line of bytes.toString('utf8').split('\n')){
    ...
    if(file==='NNG.csv'){
      if(f[0].length>=2&&f[0].length<=20){recognizedNouns.add(f[0]);if(f[5]==='행위')actionNouns.add(f[0]);if(f[5]==='상태변화')stateChangeNouns.add(f[0]);}
      continue;
    }
...
const payload=JSON.stringify({notice:'Modified by Wonboard: selected Hangul verb inflections, adverbs, recognition-only common nouns and action/state-change noun derivation subsets; stripped engine costs/IDs. Original Apache-2.0 terms: ../mecab-ko-dic/COPYING.',forms:[...forms.values()],adverbs:[...adverbs].sort(),recognizedNouns:[...recognizedNouns].sort(),actionNouns:[...actionNouns].sort(),stateChangeNouns:[...stateChangeNouns].sort()})+'\n';
...
const manifest={revision,sources,acquired:new Date().toISOString(),bytes:Buffer.byteLength(payload),gzipBytes,combinedGzipBytes:gzipBytes+existing,forms:forms.size,adverbs:adverbs.size,recognizedNouns:recognizedNouns.size,actionNouns:actionNouns.size,stateChangeNouns:stateChangeNouns.size};
