...
// Reserve one statement for the atomic checkpoint, including a 100-file document.
const MAX_BACKFILL_STATEMENTS = 13;
...
export async function backfillStoragePage(env: SitesEnv) {
  ...
  if (phase === "documents") {
    ...
    for (const row of results) {
      if (statements.length + 4 > MAX_BACKFILL_STATEMENTS) { truncated = true; break; }
      cursor = row.id;
      ...
      try {
        ...
        const files = Object.values(document.files ?? {}), metadata = JSON.stringify(files);
        if (files.length) {
          const existing = await env.DB.prepare(`SELECT count(*) AS count FROM json_each(?) j
            JOIN library_files f ON f.id=json_extract(j.value,'$.id') JOIN file_objects o ON o.id=f.object_id
            WHERE o.state='ready' AND json_extract(f.body,'$.sha256')=json_extract(j.value,'$.sha256')
              AND json_extract(f.body,'$.size')=json_extract(j.value,'$.size')`)
            .bind(metadata).first<{ count: number }>();
          if (existing?.count !== files.length) throw new Error("missingFile");
        }
        ...
        statements.push(env.DB.prepare(`INSERT OR IGNORE INTO document_file_refs
          SELECT ?,json_extract(j.value,'$.id') FROM json_each(?) j WHERE ${current}`)
          .bind(row.id, metadata, row.id, row.revision, previous.revision));
    ...
    if (!truncated && results.length < 20) { phase = "publications"; cursor = ""; }
  } else {
    ...
    for (const row of results) {
      if (statements.length + 2 > MAX_BACKFILL_STATEMENTS) { truncated = true; break; }
    ...
    if (!truncated && results.length < 20) { phase = "complete"; cursor = ""; }
  }
